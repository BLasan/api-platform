...
// UpdateSnapshot generates a new xDS snapshot from all configurations and updates the cache
// The correlationID parameter is optional and used for request tracing in logs
func (sm *SnapshotManager) UpdateSnapshotofAsyncAPI(ctx context.Context, correlationID string) error {
	// Create a logger with correlation ID if provided
	log := sm.logger
	if correlationID != "" {
		log = sm.logger.With(zap.String("correlation_id", correlationID))
	...
	// Get all configurations from in-memory store
	configs := sm.store.GetAllAsyncAPIs()

	// TODO: Improve the xds snapshot update from the current snapshot

	// currSnapShot, err := sm.cache.GetSnapshot(sm.nodeID)

	// if err != nil {
	// 	log.Error("Failed to get current snapshot", zap.Error(err))
	// 	return fmt.Errorf("failed to get current snapshot: %w", err)
	// }

	// listenerResources := currSnapShot.GetResources(resource.ListenerType)
	// clusterResources := currSnapShot.GetResources(resource.ClusterType)
	// routerResources := currSnapShot.GetResources(resource.RouteType)

	resources, err := sm.translator.TranslateAsyncConfigs(configs, correlationID)
